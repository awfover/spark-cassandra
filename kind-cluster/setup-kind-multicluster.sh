...
cat <<EOF | kind create cluster --name $cluster_name --image docker.io/kindest/node:$node_version --config=-
kind: Cluster
...
- |-
  [plugins."io.containerd.grpc.v1.cri".registry.mirrors."docker.io"]
    endpoint = ["http://docker-hub-registry:${registry_port}"]
  [plugins."io.containerd.grpc.v1.cri".registry.mirrors."registry.k8s.io"]
    endpoint = ["http://k8s-io-registry:${registry_port}"]
  [plugins."io.containerd.grpc.v1.cri".registry.mirrors."gcr.io"]
    endpoint = ["http://gcr-io-registry:${registry_port}"]
  [plugins."io.containerd.grpc.v1.cri".registry.mirrors."k8s.gcr.io"]
    endpoint = ["http://k8s-gcr-io-registry:${registry_port}"]
  [plugins."io.containerd.grpc.v1.cri".registry.mirrors."quay.io"]
    endpoint = ["http://quay-io-registry:${registry_port}"]
...
- role: worker
  kubeadmConfigPatches:
    - |
      ...
      nodeRegistration:
        kubeletExtraArgs:
          node-labels: "topology.kubernetes.io/zone=rack$((i / 2))"
  extraMounts:
  - hostPath: /home/ubun/Data/shared
    containerPath: /shared-data
    propagation: Bidirectional
  - hostPath: /home/ubun/Data/$i
    containerPath: /data
    propagation: Bidirectional
EOF2
done)
